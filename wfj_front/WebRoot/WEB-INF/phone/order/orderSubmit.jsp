...
<body>
	<form id="form1"
		action="${pageContext.request.contextPath}/phone/toPhoneSuccessOrder.action"
		method="post">
		<div id="content">
			<!--用户姓名电话地址-->
			<a href="getReceiveAddress.action?customerId=${customerId}">
				<div class="box">
					<input type="hidden" name="custAddress.customerAcceptAddressId"
						value="${custAddress.customerAcceptAddressId}">
					<p>
						<h>收货人：${custAddress.consignee}</h>
						<h>&nbsp;&nbsp;${custAddress.mobilePhone} </h>
					</p>
					<p>地址：${custAddress.address}</p>
				</div>
			</a>

			<!--商品信息-->
			<input id="shopCartIds" type="hidden" name="shopCartIds"
				value="${shopCartIds}" /> <a id="pro"
				href="${pageContext.request.contextPath}/phone/orderPro.action">
				<!-- <div class="box">
	<div class="item" style="float:left; width:40%;"><img class="proImg" width="60%" height="25%" src="../img/tuijian_38.jpg" /></div>
	<div class="item"  style="float:right; width:60%;">
		<div>宏基笔记本AAC [70G 5包/箱,80A4单包装/箱,120g]</div>
		<div class="money">￥5519.0</div>
	</div>
</div> -->
				<div class="box">
					<s:iterator value="#request.shopInfoMap" var="map">
						<s:iterator value="#map.value" var="list" status="st">
							<s:if test="#st.index==0 or #st.index==1 or #st.index==2">
								<div class="item"
									style="float: left; width: 20%; argin-right: 2%">
									<img class="proImg" width="70%" height="25%"
										src="<s:property value='#application.fileUrlConfig.visitFileUploadRoot+#list.smallImgUrl'/>" />
								</div>
							</s:if>
						</s:iterator>
					</s:iterator>
   ...
   <div class="item" style=" float:right; padding-right:5px;">
   <div > <input name="payway" type="radio" value="1"/>在线支付 </div>
   <div> <input name="payway" type="radio" value="2"/>货到付款 </div>
   </div>
  </div> -->
			...
			<div class="box">
				...
				<div class="item" style="float: right; padding-right: 5px;">
					<div>
						<input checked="checked" name="tranway" type="radio" value="1" />快递免邮
					</div>
					<div>
						<input name="tranway" type="radio" value="2" />上门自提
					</div>
				</div>
			</div>
			<!--发票信息-->
			<a href="#">
				<div class="box">
					...
					<div class="item" style="float: right; padding-right: 5px;">
						<input id="orders.invoiceType" type="hidden"
							name="orders.invoiceType" value="1">
						<div>
							纸质发票<br />个人
						</div>
						<div>非图书商品明细</div>
			...
			<div class="box">
				...
				<div class="item" style="float: right; padding-right: 5px;">
					<input type="text" name="buyerSay" value="选填，可填写您与卖家达成一致的要求"
						style="color: #999999; width: 70%; height: 30%; border: 0px"
						onblur="" />
				</div>
			</div>
			<!--商品金额，运费-->
			<div class="box" style="margin-bottom: 0px">
				<div class="item">商品金额:</div>
				<div class="item money" style="float: right; padding-right: 5px;">
					<input type="hidden" id="amount" name="orders.totalAmount"
						value="${orders.totalAmount}" /> ￥<span
						class="strong ColorRed publicPaddingL7" id="subPrice"> <fmt:formatNumber
							value="${orders.finalAmount-orders.freight}" pattern="#,##0.00#" /></span>
				</div>
			</div>
			<div class="box" style="margin-top: 0px">
				...
				<div class="item" style="float: right; padding-right: 5px;">
					<input type="hidden" id="ordersFreight" name="orders.freight"
						value="${orders.freight}" /> ￥<span id="shipping"><fmt:formatNumber
							value="${orders.freight}" pattern="#,##0.00#" /> </span>
				</div>
			</div>
		</div>
		<br /> <br /> <br />
	...
	<div class="box"
		style="width: 100%; height: 8%; background-color: #000000; color: #FFFFFF; position: fixed; bottom: 0; left: 0; margin-left: -4px">
		<div class="item"
			style="width: 65%; background-color: #000000; color: #FFFFFF;">
			实付款(含运费): <input type="hidden" id="amount" name="orders.totalAmount"
				value="${orders.finalAmount}" /> <input type="hidden"
				id="finalAmount" name="orders.finalAmount"
				value="${orders.finalAmount}" /> <span
				class="strong ColorRed publicPaddingL7" id="subtotal"> <fmt:formatNumber
