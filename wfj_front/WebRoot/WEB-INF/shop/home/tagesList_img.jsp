...
// 	    	选中品牌
		...
		function pinpai(brandId,productTypeId,level){
			...
			window.location.href="${pageContext.request.contextPath}/frontBrands/getBrandsAndProductInfos.action?brandId="+brandIds+"&productTypeId="+productTypeId+"&level="+level+"&specificationValueId="+sv+"&orderBy=normal";
		}
		//点击规格
		function guige(brandId,productTypeId,specificationValueId,level){
			...
			window.location.href="${pageContext.request.contextPath}/frontBrands/getBrandsAndProductInfos.action?brandId="+brands+"&productTypeId="+productTypeId+"&level="+level+"&specificationValueId="+sv+"&orderBy=normal";
		}
		//规格js效果
		(function($) {
			$(function(){
		...
		<div class="category_main-w search_page_layout_type1" style="float: left; width:100%;margin-bottom:30px;">
											...
											<ul class="node_list">
												...
												<s:iterator  value="#request.shopTradeTagList">
													<li class="" style="overflow: visible;width:115px;">
														...
														<a title="<s:property value='tageName'/>" href="${pageContext.request.contextPath}/homeModule/homeModule/homeModule/gotoIndustrySpecificPage.action?headerType=industrySpecific&ttId=<s:property value='ttId'/>&flagImgOrList=1" style="text-decoration: none;">
										...
										<div class="split_list">
											<ul class="node_list">
												...
												<s:iterator  value="#request.shopSituationTagList">
													<li class="" style="overflow: visible;width:115px;">
														<img src="${fileUrlConfig.fileServiceUploadRoot}shop/front/images/productList/categoryListmark.gif" width="5" height="5" />
														<a title="<s:property value='tageName'/>" href="${pageContext.request.contextPath}/homeModule/homeModule/homeModule/gotoIndustrySpecificPage.action?headerType=industrySpecific&ttId=<s:property value='ttId'/>&stId=<s:property value='stId'/>&flagImgOrList=1" style="text-decoration: none;">
															<s:if test="tageName.length()>9">
																<s:property value="tageName.substring(0,6)"/>...
															</s:if>
															<s:else>
			...
			<aside><!-- 排序 -->
					<div class="FontSize13 publicBorder publicMarginBot15 produstsList_screen ClearBoth" style="background-color: #f2f2f2;width:1182px; padding:12px 10px 11px 10px;">
						<a id="putSaleDate" href="${pageContext.request.contextPath}/homeModule/gotoIndustrySpecificPage.action?headerType=industrySpecific&flagImgOrList=1&orderBy=putSaleDateDesc&minPrice=<s:property value='minPrice'/>&maxPrice=<s:property value='maxPrice'/>&ttId=<s:property value='ttId'/>&stId=<s:property value='stId'/>">最新上架</a> 
						<a id="totalSales" href="${pageContext.request.contextPath}/homeModule/gotoIndustrySpecificPage.action?headerType=industrySpecific&flagImgOrList=1&orderBy=totalSales&minPrice=<s:property value='minPrice'/>&maxPrice=<s:property value='maxPrice'/>&ttId=<s:property value='ttId'/>&stId=<s:property value='stId'/>">销量</a> 
						<a id="salesPriceAsc" href="${pageContext.request.contextPath}/homeModule/gotoIndustrySpecificPage.action?headerType=industrySpecific&flagImgOrList=1&orderBy=salesPriceAsc&minPrice=<s:property value='minPrice'/>&maxPrice=<s:property value='maxPrice'/>&ttId=<s:property value='ttId'/>&stId=<s:property value='stId'/>" class="price_up">价格</a> 
						<a id="salesPriceDesc" href="${pageContext.request.contextPath}/homeModule/gotoIndustrySpecificPage.action?headerType=industrySpecific&flagImgOrList=1&orderBy=salesPriceDesc&minPrice=<s:property value='minPrice'/>&maxPrice=<s:property value='maxPrice'/>&ttId=<s:property value='ttId'/>&stId=<s:property value='stId'/>" class="price_down">价格</a> 
						<a href="${pageContext.request.contextPath}/homeModule/gotoIndustrySpecificPage.action?headerType=industrySpecific&flagImgOrList=2&orderBy=<s:property value='orderBy'/>&ttId=<s:property value='ttId'/>&stId=<s:property value='stId'/>" class="change_list" style="float:left;"><span style="float:left;">列表</span></a> 
						<a style="padding-top:1px;" href="${pageContext.request.contextPath}/homeModule/gotoIndustrySpecificPage.action?headerType=industrySpecific&flagImgOrList=1&orderBy=<s:property value='orderBy'/>&ttId=<s:property value='ttId'/>&stId=<s:property value='stId'/>" class="change_Gallery_select ColorRed">图片</a>
					</div>
				</aside>
              <!-- 查询的条件显示 -->
			...
			<!--Proitems-->    <!-- 商品 列表 -->
     		...
     		<form id="formModule" action="${pageContext.request.contextPath}/homeModule/gotoIndustrySpecificPage.action?headerType=industrySpecific&flagImgOrList=1&orderBy=<s:property value='orderBy'/>&ttId=<s:property value='ttId'/>&stId=<s:property value='stId'/>" method="post"> 
				...
				<ul class="WishListUl ClearBoth">
					<s:iterator value="#request.productList" var="productInfo" status="s">
					<li style="height: 335px;margin:0px 10px; position: relative; width:220px;"><a target="_blank"  style="display:block;width:220px;height:220px;" href="${pageContext.request.contextPath}/productInfo/<s:property value="#productInfo.productId" />.html">
							...
							<p style="height: 38px;overflow: hidden; margin-top:7px;">
								<a target="_blank" title="<s:property value="#productInfo.productFullName"/>" href="${pageContext.request.contextPath}/productInfo/<s:property value='#productInfo.productId'/>"><s:property value="#productInfo.productFullName" />.html</a>
							</p>
							...
							</div> <a title="<s:property value="#productInfo.shopName" />" href="${pageContext.request.contextPath}/store/frontShopMainPage/gotoShopInfoPage.action?orderBy=normal&shopInfoId=<s:property value="#productInfo.shopInfoId"/>" class="ColorBlue publicinline"><i style="background-position:left top;  width:25px !important;" class="icon iconShop01"></i>
