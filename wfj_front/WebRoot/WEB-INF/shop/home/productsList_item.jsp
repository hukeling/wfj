...
// 	    	选中品牌
		...
		function publicFunction(productTypeId,level){
			var url="${pageContext.request.contextPath}/frontBrands/list.action?orderBy=normal";
			//获取规格信息
		...
		function orderBy(parma){
			...
			window.location.href="${pageContext.request.contextPath}/frontBrands/list.action?productTypeId=${productTypeId}&orderBy="+parma+"&specificationValueId=${specificationValueId}&brandId=${brandId}&level=${level}&minPrice=${minPrice}&maxPrice=${maxPrice}&attrValueId=${attrValueId}";
		}
</script>
</head>
			...
			<table class="search_filter_table_w search_filter_table" style="margin-top: 0px;">
					<s:if test="isTrue!=true&&productTypeList1.size!=0">
						<tr id="ct" style="height:1px;">
							<th style="width:100px;">分类</th>
								<td>
									<div id="summary_category_list" class="summary_list split_line5" style="height: 40px; line-height:40px;" flag="c">
											...
											<ul class="node_list">
												...
												<s:iterator  value="productTypeList1">
													<li class="" style="overflow: visible;width:115px;">
														...
														<a title="<s:property value='sortName'/>" href="${pageContext.request.contextPath}/frontBrands/list.action?productTypeId=<s:property value="productTypeId"/>&level=<s:property value="level"/>&orderBy=normal" style="text-decoration: none;">
					...
					<s:if test="brandList.size!=0">
						<tr>
							<th>品牌</th>
							<td>
								<div id="summary_category_list" class="summary_list split_line5" style="height: auto; line-height:40px;" flag="c">
									<div class="split_list">
										<ul class="node_list">
		<!-- 										<a href="javascript:showMoreCategoryInfo();" style="text-decoration: none;">更多>></a> -->
					...
					<s:if test="isTrue==true&&#request.specificationMap.size!=0">
						<tr>
							<th>规格</th>
							<td>
								<div id="summary_category_list" class="summary_list split_line5" style="height: auto;" flag="c">
									<div class="split_list">
									<ul class="summary_list split_line5" id="jq_topmenu" style="width:800px;">
			...
			<aside><!-- 排序 -->
					<div class="FontSize13 publicBorder publicMarginBot15 produstsList_screen ClearBoth" style="background-color: #f2f2f2;width:1182px;padding:12px 10px 12px 10px;">
	...
	<ul class="account_MyCart">
		<s:iterator value="productInfoList" var="productInfo">
			...
			<img title="<s:property value="#productInfo.productFullName"/>" width="100px;" height="100px;"
				class="lazy" src="${fileUrlConfig.fileServiceUploadRoot}<s:property value='brandImageUrl'/>" data-original="<s:property value='%{#application.fileUrlConfig.visitFileUploadRoot}'/><s:property value="#productInfo.logoImg"/>" />
			</a>
			<div >
				...
				<span style="color:#d01743 ;  font-size:12px; line-height:24px;">共<s:property value="#productInfo.goodsCount" />种规格</span>
				<a target="_blank"  title="<s:property value="#productInfo.shopName"/>" href="${pageContext.request.contextPath}/store/frontShopMainPage/gotoShopInfoPage.action?orderBy=normal&shopInfoId=<s:property value="#productInfo.shopInfoId"/>&level=<s:property value="level"/>" class="ColorBlue publicinline" style="display:block;"><i class="icon iconShop01" style="background-position: left top; width:25px !important;"></i><s:property value="#productInfo.shopName" /></a>
			</div>
			<div class="ColorRed FontSize18">	 
				￥<s:property  value="#productInfo.salesPrice" /><font size="2px">起</font>
			</div>
			</li>
		</s:iterator>
	</ul>
