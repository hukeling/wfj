...
$(function(){
	...
	$('#ckb-product-all').click(function(){
		if($(this).attr('checked')=='checked'){
			$('input[name="drafts_check"]').attr('checked','checked');
		}else{
			$('input[name="drafts_check"]').removeAttr('checked');
		}
	});
	$('#delete-drafts').click(function(){
		var ckbs =$('input[name="drafts_check"]:checked');
		if(ckbs.size() == 0){
			lalert('删除','请选择消息!');
		}else{
			var ids = '';
	...
	<body>
		...
		<div class="margin-center PublicWidth1004">
			<div class="ClearBoth">
				...
				<h3 class="publicBorderB_3red FontSize16 FontSizeB publicPadding10 publicMarginBot15 ">草稿箱</h3>
				<form id="formModule" action="${pageContext.request.contextPath}/front/customer/messageCenter/listInBoxMessage.action" method="post">
					...
					<div class="FontSize13 margin-center publicBorderB" style="padding:5px 0;">
						...
						&nbsp;&nbsp;&nbsp;&nbsp; <a href="javascript:;" id="delete-drafts" class="BackgroundOrange publicinline publicPadding5_10 ColorWhite1 FontSize14">删除</a>
					</div>
						...
						<div class="publicMarginTop15">
							<table width="100%" border="0" class="RecentlyOrders_table publicMarginT10">
								...
								<s:iterator value="messageCenterList" id="m">
									<tr>
										<td>
											<input type="checkbox" name="drafts_check" value="<s:property value='#m.messageId'/>"/>
										</td>
										<td>
											<s:property value="#m.toMemberName" />
										</td>
										<td>
											<a href="${pageContext.request.contextPath}/front/customer/messageCenter/getMessageObject.action?id=<s:property value='#m.messageId'/>"><s:property value="#m.messageTitle" /></a>
										</td>
										<td>
											<s:property value="#m.createDate" />
										</td>
										<td>
											<a href="${pageContext.request.contextPath}/front/customer/messageCenter/editorMessageCenter.action?id=<s:property value='#m.messageId'/>" id="delete-drafts" class="BackgroundOrange publicinline publicPadding5_10 ColorWhite1 FontSize14">发送</a>
										</td>
									</tr>
								</s:iterator>
							</table>
