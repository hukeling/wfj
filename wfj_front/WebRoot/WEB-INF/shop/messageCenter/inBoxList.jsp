...
$(function(){
	...
	$('#ckb-product-all').click(function(){
		if($(this).attr('checked')=='checked'){
			$('input[name="inBox_check"]').attr('checked','checked');
		}else{
			$('input[name="inBox_check"]').removeAttr('checked');
		}
	});
	$('#delete-inbox').click(function(){
		var ckbs =$('input[name="inBox_check"]:checked');
		if(ckbs.size() == 0){
			lalert('删除','请选择消息!');
		}else{
			var ids = '';
			...
			window.location.href="${pageContext.request.contextPath}/front/customer/messageCenter/deleteInBoxMessageCenter.action?ids="+ids;
		}
	});
});
</SCRIPT>
	<body>
		...
		<div class="margin-center PublicWidth1004">
			<div class="ClearBoth">
				...
				<h3 class="publicBorderB_3red FontSize16 FontSizeB publicPadding10 publicMarginBot15 ">收件箱</h3>
				<form id="formModule" action="${pageContext.request.contextPath}/front/customer/messageCenter/listInBoxMessage.action" method="post">
					<div class="FontSize13 margin-center publicBorderB" style="padding:5px 0;">
						...
						&nbsp;&nbsp;&nbsp;&nbsp; <a href="javascript:;" id="delete-inbox" class="BackgroundOrange publicinline publicPadding5_10 ColorWhite1 FontSize14">删除</a>
					</div>
						...
						<div class="publicMarginTop15">
							<table width="100%" border="0" class="RecentlyOrders_table publicMarginT10">
								...
								<s:iterator value="messageCenterList" id="m">
									<tr>
										<td width="4%">
											<input type="checkbox" name="inBox_check" value="<s:property value='#m.messageId'/>"/>
										</td>
