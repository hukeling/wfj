function sHover(f, e) {
    ...
    for (var c = 0; c < d.length; c++) {
        ...
        b.size = {width: h.getStyle(b, "width"), height: h.getStyle(b, "height")};
...
sHover.prototype.hoverIn = function (c, b, d) {
    ...
    if (this.trigger(b, "in", f)) {
        ...
        if (a == 0 || a == 2) {
            ...
            d.style.left = "0px";
            this.startMove(d, {"top": 0, "left": 0, "opacity": this.opacity})
        } else {
            if (a == 1) {
            ...
            d.style.top = "0px";
            this.startMove(d, {"left": 0, "opacity": this.opacity})
        }
    }
};
sHover.prototype.hoverOut = function (d, c, f) {
    ...
    if (this.trigger(c, "out", g)) {
        ...
        switch (a) {
            case 0:
                this.startMove(f, {"top": -c.size.height, "left": 0, "opacity": b});
                break;
            case 2:
                this.startMove(f, {"top": c.size.height, "left": 0, "opacity": b});
                break;
            case 1:
                this.startMove(f, {"left": c.size.width, "top": 0, "opacity": b});
                break;
            case 3:
                this.startMove(f, {"left": -c.size.width, "top": 0, "opacity": b});
                break
...
sHover.prototype.getScroll = function () {
    var a = {top: document.body.scrollTop || document.documentElement.scrollTop, left: document.body.scrollLeft || document.documentElement.scrollLeft};
    return a
};
...
sHover.prototype.getDir = function (j, f) {
    var j = j || event;
    var b = this.getStyle(f, "width"), d = this.getStyle(f, "height"), a = (j.clientX - this.getPos(f).left + this.getScroll().left - (b / 2)) * (b > d ? (d / b) : 1), k = (j.clientY - this.getPos(f).top + this.getScroll().top - (d / 2)) * (d > b ? (b / d) : 1), g = Math.round((((Math.atan2(k, a) * (180 / Math.PI)) + 180) / 90) + 3) % 4, c = j.type;
    return g
};